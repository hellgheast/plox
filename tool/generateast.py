...
def defineVisitInterface(file:TextIOWrapper,baseName:str,types:List[str]):
    ...
    for type in types:
        ...
        print(f"{FOUR_SPACES}def visit{className}{baseName}(self,{baseName.lower()}:{className}) -> Any:",file=file)
        print(f"{2*FOUR_SPACES}raise NotImplementedError(\"Should be implemented\")\n",file=file)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lox AST generator",exit_on_error=False)
